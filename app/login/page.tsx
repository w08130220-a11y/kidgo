  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-md px-4 py-12 sm:px-6 sm:py-20">
        ...
        <Suspense fallback={<div className="text-sm text-stone-500">載入中...</div>}>
          <LoginForm />
        </Suspense>
      </main>
    </>
...
function LoginForm() {
  ...
  const errorMsg = search.get("login_error");
  ...
  if (checkingSession) {
    return <div className="text-center text-sm text-stone-500">檢查登入狀態...</div>;
  }
    ...
    <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      {errorMsg && (
        <div className="mt-5 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs text-rose-900">
          登入失敗：{decodeURIComponent(errorMsg)}
        </div>
      )}
